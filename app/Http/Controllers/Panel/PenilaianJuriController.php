...
class PenilaianJuriController extends Controller
{
    ...
    {
        //Hanya Juri GK yang bisa mengakses.
        if (Auth::user()?->jenis_juri !== 'GK') {
            abort(403, 'Anda tidak memiliki akses ke halaman penilaian Gagasan Kreatif.');
        }
    ...
    {
        //Hanya Juri GK yang bisa menyimpan nilai.
        if (Auth::user()?->jenis_juri !== 'GK') {
            abort(403, 'Anda tidak diizinkan untuk menyimpan nilai Gagasan Kreatif.');
        }
        $periodeAktif = TahunSeleksi::where('is_active', true)->first();
        if (!$periodeAktif || $periodeAktif->status != 'penilaian') {
    ...
    {
        //Hanya Juri BI yang bisa mengakses.
        if (Auth::user()?->jenis_juri !== 'BI') {
            abort(403, 'Anda tidak memiliki akses ke halaman penilaian Bahasa Inggris.');
        }
    ...
    {
        //Hanya Juri BI yang bisa menyimpan nilai.
        if (Auth::user()?->jenis_juri !== 'BI') {
            abort(403, 'Anda tidak diizinkan untuk menyimpan nilai Bahasa Inggris.');
        }
