...
@section('content')
        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    ...
                    <p class="text-subtitle text-muted">Upload dan kelola berkas untuk:
                ...
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Manajemen Berkas</li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <section class="section">
            ...
            @if ($periodeAktif && $periodeAktif->status != 'pendaftaran')
                <div class="alert alert-info">
                    ...
                    <p>Periode ini sedang dalam tahap '{{ $periodeAktif->status }}'. Anda hanya dapat melihat berkas dan tidak dapat melakukan perubahan apa pun.</p>
                </div>
            @endif

            {{-- Menampilkan peringatan jika CU belum diunggah (diambil dari kode lama yang lebih lengkap) --}}
            ...
            <div class="row">
                {{-- Kolom Kiri: Informasi dan Aksi Panitia (Menggunakan versi baru yang lebih rapi) --}}
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-body">
                            {{-- BAGIAN PROFIL YANG DIRAPIKAN --}}
                            <div class="d-flex flex-column align-items-center text-center py-3">
                                <div class="avatar shadow-sm mb-3" style="width: 120px; height: 120px;">
                                    <img src="{{ $peserta->foto_path ? asset('storage/' . $peserta->foto_path) : asset('assets/compiled/jpg/1.jpg') }}" 
                            ...
                            <div class="d-grid gap-2">
                                @if (($peserta->status_verifikasi == 'menunggu' || $peserta->status_verifikasi == 'ditolak') && $periodeAktif && $periodeAktif->status == 'pendaftaran')
                                    ...
                                    @if(!$peserta->berkas_lengkap)
                                        <div class="form-text text-center text-danger mt-1">
                                            ...
                                            Tombol validasi aktif setelah semua berkas wajib diunggah.
                                        </div>
                                    @endif
                                @endif
                                ...
                                @if ($periodeAktif && $periodeAktif->status == 'pendaftaran')
                                    <button class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#editPesertaModal"
                                        data-id="{{ $peserta->id }}" data-nama_lengkap="{{ $peserta->nama_lengkap }}"
                                ...
                                @if ($peserta->status_verifikasi != 'ditolak')
                                    <button type="button" class="btn btn-danger w-100 mt-2" data-bs-toggle="modal" data-bs-target="#tolakPendaftaranModal">
                                        <i class="bi bi-trash-fill"></i> Tolak Pendaftaran
                                    </button>
                                @endif
                            </div>
                        </div>
                ...
                {{-- Kolom Kanan: Daftar Berkas (Diambil dari kode lama yang lengkap) --}}
                <div class="col-md-8">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="accordion" id="accordionBerkas">
                                ...
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="headingBerkasWajib"><button class="accordion-button"
                                            type="button" data-bs-toggle="collapse"
                                            data-bs-target="#collapseBerkasWajib">Berkas Wajib</button></h2>
                                                                    ...
                                                                    <div class="d-inline-flex gap-1">
                                                                        ...
                                                                        @if ($periodeAktif && $periodeAktif->status == 'pendaftaran')
                                                                            <button type="button"
                                                                                ...
                                                                                data-bs-target="#uploadBerkasModal"
                                                                                data-nama-berkas="{{ $nama }}"
                                                                                data-update-url="{{ route('panel.berkas.update', $file->id) }}">Ganti</button>
                                                                            <form action="{{ route('panel.berkas.destroy', $file->id) }}" method="POST" class="m-0 form-delete"
                                                                                data-confirm-text="Hapus berkas {{ $nama }}?">
                                                                                @csrf @method('DELETE')
                                                                ...
                                                                @else
                                                                    @if ($periodeAktif && $periodeAktif->status == 'pendaftaran')
                                                                        <button class="btn btn-primary btn-sm"
                                                                            data-bs-toggle="modal"
                                                                            data-bs-target="#uploadBerkasModal"
                                                                            data-jenis-berkas="{{ $key }}"
                                                                            data-nama-berkas="{{ $nama }}"
                                                                            data-store-url="{{ route('panel.berkas.store', $peserta->id) }}">Upload</button>
                                                                    @endif
                                                                @endif
                                                            </td>
                                                        </tr>
                                ...
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="headingCu">
                                        <button class="accordion-button collapsed" type="button"
                                            data-bs-toggle="collapse" data-bs-target="#collapseCu" aria-expanded="false"
                                            aria-controls="collapseCu">
                                            <i class="bi bi-award-fill me-2"></i>
                                            Bukti Capaian Unggulan (Maks. 10)
                                            <span class="badge bg-info ms-auto">
                                                {{ $berkas->get('CU')?->count() ?? 0 }}/10 Terupload
                                            </span>
                                        </button>
                                    </h2>
                                                ...
                                                <tbody>
                                                    @forelse ($berkas->get('CU') ?? [] as $cu)
                                                        <tr>
                                                            ...
                                                            <td><span
                                                                    class="badge bg-light-secondary">{{ $cu->tingkat }}</span>
                                                            </td>
                                                            <td><span class="badge bg-success">Tersedia</span></td>
                                                            <td>
                                                                <div class="d-inline-flex gap-1">
                                                                    <a href="{{ asset('storage/' . $cu->path_file) }}"
                                                                        target="_blank"
                                                                        class="btn btn-info btn-sm">Lihat</a>
                                                                    @if ($periodeAktif && $periodeAktif->status == 'pendaftaran')
                                                                        <button type="button"
                                                                            class="btn btn-warning btn-sm"
                                                                            data-bs-toggle="modal"
                                                                            data-bs-target="#editCuModal"
                                                                            data-update-url="{{ route('panel.berkas.update', $cu->id) }}"
                                                                            data-nama-berkas="{{ $cu->nama_berkas }}"
                                                                            data-tingkat="{{ $cu->tingkat }}">
                                                                            Edit
                                                                        </button>
                                                                        <form action="{{ route('panel.berkas.destroy', $cu->id) }}" method="POST" class="m-0 form-delete"
                                                                            data-confirm-text="Hapus capaian unggulan '{{ $cu->nama_berkas }}'?">
                                                                            @csrf @method('DELETE')
                                                                            <button type="submit" class="btn btn-danger btn-sm">Hapus</button>
                                                                        </form>
                                                                    @endif
                                                    ...
                                                    @empty
                                                        <tr>
                                                            <td colspan="4" class="text-center text-muted">
                                                                Belum ada Capaian Unggulan yang diunggah.
                                                            </td>
                                                        </tr>
                                                    @endforelse
                                                    ...
                                                    @if ($periodeAktif && $periodeAktif->status == 'pendaftaran')
                                                        @for ($i = $berkas->get('CU')?->count() ?? 0; $i < 10; $i++)
                                                            <tr>
                                                                <td class="text-muted">Capaian Unggulan
                                                                    ({{ $i + 1 }}/10)</td>
                                                                <td></td>
                                                                ...
                                                                <td>
                                                                    <button class="btn btn-primary btn-sm"
                                                                        data-bs-toggle="modal"
                                                                        data-bs-target="#uploadBerkasModal"
                                                                        data-jenis-berkas="CU"
                                                                        data-nama-berkas="Capaian Unggulan ({{ $i + 1 }}/10)"
                                                                        data-store-url="{{ route('panel.berkas.store', $peserta->id) }}">
    ...
    @endif
    @if ($peserta->status_verifikasi != 'ditolak')
        @include('panel.peserta._tolak-pendaftaran-modal')
    @endif
@endsection

@push('scripts')
    {{-- Diambil dari kode sumber --}}
