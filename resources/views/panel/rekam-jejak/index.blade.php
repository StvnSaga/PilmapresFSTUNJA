        ...
        <section class="section">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <hr>
                    <form id="form-rekam-jejak" method="POST" action="{{ route('rekam-jejak.store') }}" enctype="multipart/form-data">
                        @csrf
                                ...
                                <div class="col-md-4">
                                    <div class="card bg-light border winner-form-card rank-{{ $peringkat }} h-100">
                                        <div class="card-body d-flex flex-column">
                                            <h5 class="text-center">Peringkat {{ $peringkat }}: <strong>{{ $peserta->nama_lengkap }}</strong></h5>
                                            <p class="text-center text-muted mb-3">Skor Akhir: {{ number_format($peserta->skor_akhir, 2) }}</p>

                                            <input type="hidden" name="pemenang[{{ $peserta->id }}][peringkat]" value="{{ $peringkat }}">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="winner-photo-preview mb-2">
                                                    ...
                                                    @php
                                                        $fotoTampil = optional($pemenangData)->foto_path ?? $peserta->foto_path;
                                                    @endphp
                                                ...
                                                <input type="file" name="pemenang[{{ $peserta->id }}][foto]" class="form-control form-control-sm" accept="image/*">
                                                <small class="text-muted">Kosongkan jika ingin menggunakan foto profil utama peserta.</small>
                                            </div>

                                            <div class="form-group mt-auto">
                                                <label>Deskripsi Singkat</label>
                                                <textarea name="pemenang[{{ $peserta->id }}][deskripsi]" class="form-control" rows="3" placeholder="Contoh: Meraih Juara {{ $peringkat }} setelah presentasi yang memukau...">{{ optional($pemenangData)->deskripsi_singkat ?? '' }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="text-center mt-4">
                            <button type="submit" class="btn btn-primary btn-lg">Simpan & Tampilkan di Rekam Jejak</button>
                        </div>
                    </form>
